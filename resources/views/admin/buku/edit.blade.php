          ...
          <div class="card-body">
            <form action="{{ route('buku.update',$buku) }}" method="post" enctype="multipart/form-data">
              ...
              <div class="row">
                  <div class="col-md-6">
                      <div class="form-group">
                          ...
                          <input type="text" value="{{ $buku->judul ?? old('judul') }}" class="form-control @error('judul') is-invalid @enderror" name="judul" placeholder="Masukkan judul buku" value="{{ old('judul') }}">
                        ...
                        <label class="form-control-label">{{ __('Nama Penulis') }}</label>
                          ...
                          @foreach ($penulis as $item)
                            <option value="{{ $item->id }}"
                              {{ $item->id == $buku->penulis_id ? 'selected' : '' }}>
                                {{ $item->name }}</option>
                        @endforeach
                          </select>
              ...
              <div class="row">
                  <div class="col-md-6">
                      <div class="form-group">
                          ...
                          <textarea  name="keterangan"  cols="30" rows="2" class="form-control @error('keterangan') is-invalid @enderror" placeholder="Tuliskan deskripsi buku">{{ $buku->keterangan ?? old('keterangan') }}</textarea>
                  ...
                  <div class="col-md-6">
                      <div class="form-group">
                          ...
                          <input type="number" class="form-control @error('jumlah') is-invalid @enderror" name="jumlah" placeholder="jumlah" value="{{ $buku->jumlah ?? old('jumlah') }}">
              ...
              <div class="d-flex justify-content-start">
                  <button type="submit" class="btn bg-gradient-primary btn-md mt-4 mb-4">{{ 'Ubah' }}</button>
              </div>
          </form>  
          </div>
